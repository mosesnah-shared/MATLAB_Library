...
%                 end
        ...
        function goto( obj, idx )
            ...
            if iidx ~= 0
                for tmp = iidx 
                set( obj.hAxes{ tmp },  'XLim',  [  -obj.zoomSize( tmp ) + obj.graphicObjs{ tmp }( obj.zoomIdx( tmp ) ).XData( idx ), obj.zoomSize( tmp ) + obj.graphicObjs{ tmp }( obj.zoomIdx( tmp ) ).XData( idx ) ] , ...         
                                         'YLim',  [ -obj.zoomSize( tmp ) + obj.graphicObjs{ tmp }( obj.zoomIdx( tmp ) ).YData( idx ), obj.zoomSize( tmp ) + obj.graphicObjs{ tmp }( obj.zoomIdx( tmp ) ).YData( idx ) ] , ...    
                                         'ZLim',  [ -obj.zoomSize( tmp ) + obj.graphicObjs{ tmp }( obj.zoomIdx( tmp ) ).ZData( idx ), obj.zoomSize( tmp ) + obj.graphicObjs{ tmp }( obj.zoomIdx( tmp ) ).ZData( idx ) ] )  
...
%             plot( obj.hAxes{ idx }, marker.XData, marker.YData, 'color', marker.MarkerEdgeColor, 'linewidth', marker.LineWidth )
        ...
        function addZoomWindow( obj, idx, whichMarker, size )
            ...
            obj.zoomIdx( idx ) = idxM; 
            obj.zoomSize( idx ) = size;
